...
def run_backtest_and_save(csv_paths: list, interval: str, config: dict, output_path: str = None):
    ...
    strategy = SplitTakeProfitStrategy(
        ...
        stop_loss_rate=entry_config.get('stop_loss_rate', 0.03),
        take_profit_count=entry_config.get('take_profit_count', 5),
        first_take_profit_rate=entry_config.get('first_take_profit_rate', 0.02),
        take_profit_interval=entry_config.get('take_profit_interval', 0.01)
    )

    result = strategy.run_split_backtest(klines_df, indicators, Decimal("10000"))
...
def analyze_orders(result: dict):
    ...
    print(f"\n止盈订单数: {len(take_profit_orders)}")
    print(f"止损订单数: {len(stop_loss_orders)}")
    print(f"总订单数: {len(completed_orders)}")

    if completed_orders:
        print(f"止盈率: {len(take_profit_orders)/len(completed_orders)*100:.1f}%")
        print(f"止损率: {len(stop_loss_orders)/len(completed_orders)*100:.1f}%")
    ...
    # 按止盈份数统计
    portion_stats = defaultdict(list)
    for order in take_profit_orders:
        portion_index = order.get('portion_index', 1)
        portion_stats[portion_index].append(order)

    print(f"\n各止盈档位成交情况:")
    print(f"{'档位':<8} {'止盈比例':<12} {'成交次数':<10} {'平均盈利':<15}")
    print("-" * 50)

    take_profit_rates = [2, 3, 4, 5, 6]
    for i in range(1, 6):
        orders = portion_stats.get(i, [])
        count = len(orders)
        avg_profit = sum(o.get('profit_loss', 0) for o in orders) / count if count > 0 else 0
        print(f"第{i}档      {take_profit_rates[i-1]}%          {count:<10} {avg_profit:.4f}")
    ...
    tp_pnl = sum(o.get('profit_loss', 0) for o in take_profit_orders)
    sl_pnl = sum(o.get('profit_loss', 0) for o in stop_loss_orders)

    print(f"总成交订单: {len(completed_orders)}")
    print(f"止盈订单: {len(take_profit_orders)} (盈利: {tp_pnl:.2f} USDT)")
    print(f"止损订单: {len(stop_loss_orders)} (亏损: {sl_pnl:.2f} USDT)")
    ...
    print("【5. 持仓统计】")
    print("=" * 70)
    print(f"剩余持仓数: {result['remaining_holdings']}")
    print(f"持仓成本: {result['holding_cost']:.2f} USDT")
    print(f"持仓市值: {result['holding_value']:.2f} USDT")
    print(f"持仓浮盈浮亏: {result['holding_unrealized_pnl']:.2f} USDT")

    # ============ 6. 与策略12对比 ============
    print("\n" + "=" * 70)
    print("【6. 策略对比参考】")
    print("=" * 70)
    print("策略12 (固定2%止盈, 6%止损):")
    print("  - 收益率: 0.72%")
    print("  - 胜率: 93.1%")
    print("")
    print(f"策略13 (分批止盈2%-6%, 3%止损):")
    print(f"  - 收益率: {result['return_rate']:.2f}%")
...
if __name__ == '__main__':
    ...
    # 加载配置
    config_path = 'strategy_adapter/configs/strategy13_split_take_profit.json'
    ...
    # 运行回测并保存订单
    output_path = 'strategy_adapter/tests/strategy13_orders.json'
