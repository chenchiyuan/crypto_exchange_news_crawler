"""
检测所有监控合约的规则6和7触发情况
Temporary command for validation
"""
...
class Command(BaseCommand):
    help = '检测所有监控合约的规则6和7触发情况'

    def handle(self, *args, **options):
        self.stdout.write('=' * 80)
        self.stdout.write('规则6和7实时检测 - 所有监控合约')
        self.stdout.write('=' * 80)
        self.stdout.write('')
        ...
        # 获取所有启用的监控合约
        contracts = MonitoredContract.objects.filter(status='enabled').order_by('symbol')
        ...
        for idx, contract in enumerate(contracts, 1):
            ...
            self.stdout.write(f'[{idx}/{total_count}] 检测 {symbol}...', ending=' ')
            self.stdout.flush()
            ...
            try:
                ...
                klines_4h = cache.get_klines(symbol, interval='4h', limit=42, use_cache=False)
        ...
        if rule_6_triggers:
            for trigger in rule_6_triggers:
                self.stdout.write(f"  {trigger['symbol']:12s} ${float(trigger['price']):>10,.2f}")
        ...
        self.stdout.write('=' * 80)
        self.stdout.write(f'总结: 检测 {total_count} 个合约，规则6触发 {len(rule_6_triggers)} 个，规则7触发 {len(rule_7_triggers)} 个')
        self.stdout.write('=' * 80)
