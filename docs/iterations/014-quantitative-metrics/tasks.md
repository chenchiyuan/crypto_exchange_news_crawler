...
- **验收标准**:
  - [x] 在`add_arguments()`方法中新增--risk-free-rate参数 ✅
  - [x] 参数类型：float，默认值：3.0 ✅
  - [x] 参数help说明：无风险收益率（年化，百分比），默认3.0% ✅
  - [x] 参数传递给MetricsCalculator构造函数 ✅ **将在TASK-014-011中集成**
  - [x] 测试通过：未指定时使用默认值3% ✅
  - [x] 测试通过：指定--risk-free-rate 5.0时使用5% ✅
  - [x] **异常路径验证**:
    - [x] 参数值超出合理范围[0, 100]时，显示警告（不阻止执行） ✅
  - [x] **文档化标准合规** 📝
    - [x] 更新help文档，说明参数含义和常见值 ✅
- **边界检查**:
...
- **文档要求**: ⭐
  ...
  - **逻辑上下文注释**: 参数业务含义注释
- **状态**: ✅ 已完成
- **提交**: de1aa92 - feat(TASK-014-010): 添加 --risk-free-rate CLI 参数
- **测试结果**: 4 passed in 0.03s
