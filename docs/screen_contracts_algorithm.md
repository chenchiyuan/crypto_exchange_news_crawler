...
这是一个**短周期高频波动**指标，用来衡量币价在短时间内的"活跃程度"。
...
| NATR | 绝对波动率大小 | 5%-10% | 14根4h K线 | 确保有利润空间 |
| KER | 趋势效率（越低越震荡）| < 0.4 | 10根4h K线 | 排除单边趋势 |
| 15m振幅累计 | 短周期高频活跃度 | 50%-150% | 100根15m K线（25小时） | 确认微观波动 |

**四个指标的协同关系**：

```
VDR高（>10） + NATR高（>5%） + KER低（<0.3） + 15m振幅高（>80%）
= 宏观震荡 + 绝对波动大 + 无明显趋势 + 微观活跃
= 完美的震荡网格标的！
```
...
- **NATR**：从"幅度"判断波动是否足够（中观）
- **15m振幅累计**：从"频率"判断是否高频活跃（微观）
...
| 15m振幅累计 | 15-min Amplitude Sum | 100根15m K线 | 50%-150% | 短周期活跃度 |
