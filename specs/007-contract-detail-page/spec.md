...
- **数据缺失场景**: 如果某个合约的K线数据不足(如新上市合约只有10根4h K线)，系统如何展示？是显示不足的K线并提示数据不完整，还是拒绝展示图表？
- **URL直接访问**: 用户直接在浏览器输入详情页URL(如/screening/daily/2024-12-05/BTCUSDT)，系统如何验证该日期该合约是否存在筛选记录？如果不存在，是返回404还是显示友好提示页面？
- **指标计算失败**: 如果某个合约的某些指标计算失败(返回None或0)，在详情页如何显示？是显示"N/A"、"--"还是隐藏该指标？
- **图表性能**: 当K线数量很大(如300根)且叠加多条指标线时，图表渲染性能如何？是否需要虚拟化或分批加载？
- **移动端适配**: 在移动设备上查看K线图时，如何保证图表的交互性(缩放、拖动、tooltip显示)？
...
- **FR-019**: 如果K线数据不足指定根数(如新合约只有20根4h K线)，系统必须显示实际存在的所有K线，并在图表上方显示警告提示"数据不足，仅显示XX根K线"
...
- **FR-025**: K线图渲染时间必须在1秒以内
- **FR-026**: 系统必须提供加载指示器，在数据获取和图表渲染期间显示"加载中..."状态
- **FR-027**: 如果访问的日期或合约不存在筛选记录，系统必须显示友好的404页面，提供返回列表和查看其他日期的快捷链接
